...
{
    //1.获取数组指针
    jshort *inputArray = (*env)->GetShortArrayElements(env, data, NULL);
    ...
    //4.释放资源
    (*env)->ReleaseShortArrayElements(env, data , inputArray, JNI_COMMIT);
    ...
    get_micro_audio_frontend(inputArray, arr);
#if 0
    for (int i=0; i<40; i++) {
        LOGI("%f", arr[i]);
    }
