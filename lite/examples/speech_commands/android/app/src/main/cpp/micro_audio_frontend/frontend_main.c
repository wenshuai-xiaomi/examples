...
// based on the wave file structure protocol: specific header(44) + data
  size_t audio_file_size = ftell(fp) / sizeof(int16_t) - 22;
  fseek(fp, 44L, SEEK_SET);

// read PCM data to allocated memory
  int16_t* audio_data = malloc(audio_file_size * sizeof(int16_t));
// pointer audio_data will be changed to get new data, so add another variable
...
#if (FIXED_POINT == 16)
  printf("check the meaning of add_definitions\n");
#endif
