...
bool Spectrogram::Initialize(const std::vector<double>& window,
  ...
  if (window_length_ < 2) {
    std::cout << "Window length too short.";
    initialized_ = false;
    return false;
  }
  ...
  if (step_length_ < 1) {
    std::cout << "Step length must be positive.";
    initialized_ = false;
    return false;
  }

  fft_length_ = NextPowerOfTwo(window_length_);
  if (fft_length_ < window_length_) {
    std::cout << "ERROR\n";
    return EXIT_FAILURE;
  }
...
bool Spectrogram::Reset() {
  if (!initialized_) {
    std::cout << "Initialize() has to be called, before Reset().";
    return false;
  }
...
bool Spectrogram::ComputeComplexSpectrogram(
  ...
  if (!initialized_) {
    std::cout << "ComputeComplexSpectrogram() called before successful call "
               << "to Initialize().";
    return false;
  }
  if (output == nullptr) {
  ...
  while (GetNextWindowOfSamples(input, &input_start)) {
    if (input_queue_.size() != window_length_) {
      std::cout << "Error\n";
      return EXIT_FAILURE;
    }
...
bool Spectrogram::ComputeSquaredMagnitudeSpectrogram(
  ...
  if (!initialized_) {
    std::cout << "ComputeSquaredMagnitudeSpectrogram() called before "
  ...
  int input_start = 0;
  while (GetNextWindowOfSamples(input, &input_start)) {
    if (window_length_ != input_queue_.size()) {
      std::cout << "Not equal\n";
      return EXIT_FAILURE;
    }
    ProcessCoreFFT();  // Processes input_queue_ to fft_input_output_.
